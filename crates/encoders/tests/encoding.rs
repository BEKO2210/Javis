...
fn shared_word_yields_exact_overlap() {
    ...
    assert_eq!(
        hw.overlap(&hello),
        K as usize,
        "hello word missing in 'hello world'"
    );
    assert_eq!(
        hr.overlap(&hello),
        K as usize,
        "hello word missing in 'hello rust'"
    );

    // The overlap of the two texts is at least the "hello" part.
    let hw_hr = hw.overlap(&hr);
    assert!(hw_hr >= K as usize, "expected overlap ≥ k={K}, got {hw_hr}",);
...
fn punctuation_is_stripped() {
    ...
    assert_eq!(
        a.indices, b.indices,
        "punctuation should not change the SDR"
    );
}
